...
def parse_one_bag(path):
    ...
    start = bag.read_messages(topics=['/'+quad_name+'/if_started'])
    ...
    #         start_t = t
    #     else:
    #         start_t = min(t, start_t)
        

    # end = bag.read_messages(topics=['/'+quad_name+'/flight_pilot/if_end'])
    end = bag.read_messages(topics=['/'+quad_name+'/if_reached'])
    ...
    for topic, msg, t in end:
        ...
        if len(end_deque)>2:
            if end_deque[2]<reach_radius and end_deque[1]>=reach_radius:
                end_t = t
                break
    # # take the first end time
    # end = bag.read_messages(topics=['/'+quad_name+'/if_end'])
    # for topic, msg, t in end:
    #     if end_t is None:
    #         end_t = t
    #     else:
    ...
    collision = bag.read_messages(topics=['/'+quad_name+'/if_collision'])
    for topic, msg, t in collision:
        if t>start_t and t<end_t:
            # collided within valid time
            if msg.data == True:
                print("collision at: ", (t-start_t).to_sec())
                success = False
                return output()
        # collided outside valid time dont care
    ...
    # TODO: Ignore this case for the time being
    # check if planner stops planning
    # cmd = bag.read_messages(topics=['/'+quad_name+'/planning/pos_cmd'])
    # last_cmd_t = None
    # for topic, msg, t in cmd:
    #     if last_cmd_t is None:
    #         last_cmd_t = t
    #     else:
    #         last_cmd_t = max(last_cmd_t, t)
    # if (last_cmd_t - end_t).to_sec() > plan_after_reach_time:
    #     # TODO: handle this case
    #     print("Still planning after reached goal")
    ...
    # autopilot_fb = bag.read_messages(topics=['/'+quad_name+'/ground_truth/odom'])
    autopilot_fb = bag.read_messages(topics=['/'+quad_name+'/odom']) # currently only have odom, GT odom = odom
    for topic, msg, t in autopilot_fb:
        ...
        point_deque.append(np.array([msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z]))
        if t>start_t and t<end_t:
            ...
            speed = np.linalg.norm(np.array([msg.twist.twist.linear.x, msg.twist.twist.linear.y, msg.twist.twist.linear.z]))
            if speed>max_speed_:
    ...
    imu = bag.read_messages(topics=['/'+quad_name+'/imu']) # imu same as GT imu
    jerk_list = []
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("\033[31mPlease use: 'python3 bag_paser_.py <bag_path> <test_case_id> <algorithm_name>' to parse bags.\033[0m")
        exit(-1)
    else:
        bag_list = os.listdir(sys.argv[1])
        parse_list = []
        for name in bag_list:
            if sys.argv[3]+"_test_"+sys.argv[2] in name:
