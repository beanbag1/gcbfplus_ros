...
class LinearDrone(MultiAgentEnv):
    ...
    def step(
        ...
        assert agent_states.shape == (self.num_agents, self.state_dim)
        #actions is ax, ay, az
        #agent state is px, py, pz, vx, vy, vz 
        #jax.debug.print("action: {a}", a=action)
        #jax.debug.print("agent states: {b}", b=agent_states)
    ...
    def get_graph(self, state: EnvState, lidar_data, adjacency: Array = None) -> GraphsTuple:
        ...
        #     )
        # )
        # vmap_lidar_data = merge01(get_lidar_vmap(state.agent[:, :3]))
        # print(type(vmap_lidar_data))
        ...
        # jax.debug.print("states: {a}", a=jnp.concatenate([state.agent, state.goal, lidar_data], axis=0))
        # create graph
        return GetGraph(
            nodes=node_feats,
            node_type=node_type,
