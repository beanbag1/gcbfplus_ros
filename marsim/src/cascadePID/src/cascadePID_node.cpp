...
void publishCurrentVel(const ros::TimerEvent& event) 
{
    Eigen::Vector3d current_vel = quad_PID.getCurrentVel();
    std_msgs::Float32MultiArray vel_array;
    vel_array.data.clear();
    vel_array.data.push_back(current_vel(0));
    vel_array.data.push_back(current_vel(1));
    vel_array.data.push_back(current_vel(2));
    current_vel_pub.publish(vel_array);
}

int main(int argc, char** argv)
{
    ros::init(argc, argv, "cascade_PID");
    ROS_INFO("cascadePID node");
    ...
    current_vel_pub = n.advertise<std_msgs::Float32MultiArray>("current_vel", 100);
    ...
    ros::Timer vel_timer = n.createTimer(ros::Duration(1.0/controller_rate), publishCurrentVel);
